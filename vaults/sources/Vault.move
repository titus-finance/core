module titusvaults::Vault {
    ...
    public fun create_vault<VaultT, AssetT>(_host: &signer) acquires Round {
        let host_addr = address_of(_host);
        assert!(host_addr == @titusvaults, E_NOT_AUTHORIZED);
    ...
    public (friend) fun deposit_vault<VaultT, AssetT>( account: &signer, _coin: Coin<AssetT>) acquires Vault, VaultMap, Round {
        ...
        if (smart_table::contains(&vault_map.deposits, user_addr)) {
            ...
            smart_table::add(&mut vault_map.rounds, user_addr, current_round.round);  
        };
    ...
    fun test_vault(titusvaults: &signer, user: &signer) acquires Round{
        setup();
        set_current_time(titusvaults);
    ...
    // test deposit
    #[test(titusvaults = @0xa2bf272177d0723a90cb462bb087309e84554245934445af0da21132ba798b59, user = @0x2)]
    fun test_deposit(titusvaults: &signer, user: signer) acquires Vault, VaultMap, Round {
        setup();
        set_current_time(titusvaults);
    ...
    #[test(titusvaults = @0xa2bf272177d0723a90cb462bb087309e84554245934445af0da21132ba798b59, user = @0x2)]
    fun test_instant_withdraw_vault(titusvaults: &signer, user: signer) acquires Round, Vault, VaultMap {
        setup();
        set_current_time(titusvaults);
    ...
    #[expected_failure]
    fun test_standard_withdraw_vault(titusvaults: &signer, user: signer) acquires Round, Vault, VaultMap {
        setup();
        set_current_time(titusvaults);
